...
const OurTeam = ({ schoolCode }) => {
        ...
        const fetchStaffs = async () => {
            try {
                ...
                const response = await fetch(
                    `https://zuss-school-management-system-server-site.vercel.app/api/staffs/${schoolCode}`
                );
                if (response.ok) {
                    const staffsData = await response.json();
                    setStaffs(staffsData);
    ...
    const groupStaffsByStatus = () => {
        ...
        staffs.forEach((staff) => {
            const status = staff?.selectedStatus || 'Others'; // Use 'Others' as the default if selectedStatus is undefined

            if (status === 'Management') {
                groupedStaffs.Management.push(staff);
            } else if (status === 'Principal') {
                groupedStaffs.Principal.push(staff);
            } else if (status === 'Teacher') {
                groupedStaffs.Teacher.push(staff);
            } else {
                groupedStaffs.Others.push(staff);
            }
        });
    ...
    const groupedStaffs = groupStaffsByStatus();
        ...
        <section className="pt-32 pb-12 dark:bg-gray-800 dark:text-gray-100">
            <div className="container p-4 mx-auto space-y-16 sm:p-10">
                <div className="space-y-4 mx-auto">
                    <h3 className="text-2xl font-bold leading-none sm:text-5xl">Meet our school team</h3>
                    <p className="">
                        Teachers and other staffs of a school change the students' lives and make them what they are today. Our team became a successful team, teacher, as a result of having top-notch teachers, grade school 1–6 grade, providing a solid foundation for students to build all future educational endeavors.
                    </p>
                </div>
                <div className="grid w-full ">
                    {Object.entries(groupedStaffs).map(([status, staffList]) => (
                        <div key={status}>
                            <h2 className="text-3xl font-bold mt-20 mb-10 underline text-lime-200 ">{status}</h2>
                            <div className=" grid grid-cols-3 gap-4">


                                {staffList.length > 0 && staffList.map((staff) => (
                                    <div key={staff?._id} className="space-y-4">
                                        <img alt="" className="object-cover h-56 mx-auto mb-4 bg-center rounded-sm dark:bg-gray-500" src={staff?.image} />
                                        <div className="flex flex-col items-center">
                                            <h4 className="text-xl font-semibold">{staff?.name}</h4>
                                            <p className="text-sm dark:text-gray-400">{staff?.designation}</p>
